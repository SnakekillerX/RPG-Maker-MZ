...
@help
========================================================================
Modified by SnakekillerX to add the ability to use Dynamic Plugin Names.
▪ Old Example:

pluginName = 'X_SNK_PluginName';
param = PluginManager.parameters(pluginName);

▪ New Example:
if(window.currentPluginProcessed){
	pluginName = window.currentPluginProcessed;
	param = PluginManager.parameters(pluginName)
}
========================================================================

FOSSIL goes at the start, before all other plugins.
...
fossilStaticFixes = function(){
	...
	PluginManager.registerCommand('FOSSIL_X_SNK', 'useOldPlugin' , args => {
...
/* 		if((typeof(Yanfly)=='undefined')&&!(typeof(window.Yanfly)=='undefined'))
	...
	{
		if(documentBodyList[elementIndex].outerHTML.contains('plugins/FOSSIL_X_SNK.js'))
		{
...
if(typeof(scriptUrls)!=="undefined")
{
	...
	if($plugins[0].name!=='FOSSIL_X_SNK')
	{
	...
	writeNewIndexFile = function(rawIndexHtml) {
		...
		rawIndexHtml = rawIndexHtml.replace("main.js", "plugins/FOSSIL_X_SNK.js");
		...
		PluginManager.setup = function(plugins) {
			...
			for (const plugin of plugins) {
				...
				if (plugin.status && !this._scripts.includes(plugin.name)) {
					...
					PluginManager.parameters(plugin.name).ThisPluginName = plugin.name;
					}
				}
			}
