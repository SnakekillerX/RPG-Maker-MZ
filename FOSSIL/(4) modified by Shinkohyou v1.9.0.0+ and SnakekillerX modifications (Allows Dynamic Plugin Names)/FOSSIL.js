...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Fossil Core Engine
//=============================================================================
// Date: 2025/08/10
// Reason: A collection of essential static patches for FOSSIL. It provides
// full subfolder compatibility for the plugin manager and implements a robust
// handler for legacy MV plugin commands, ensuring cross-compatibility with MZ.
//-----------------------------------------------------------------------------
fossilStaticFixes = function() {
    ...
    if (Fossil.chattyOutput) {
        console.log('FOSSIL: Applying initial static patches.');
    }
    ...
    // --- COMPLETE SUBFOLDER COMPATIBILITY PATCH ---
    (function() {
        if (DataManager.isFossilPatchedForSubfolders) return;
        DataManager.isFossilPatchedForSubfolders = true;

        // PART 1: Patch the global 'Imported' object.
        // Reason: This allows Fossil to "see" plugins located in subfolders, which is
        // critical for compatibility checks (e.g., `Imported.YEP_CoreEngine`).
        const _PluginManager_setup = PluginManager.setup;
        PluginManager.setup = function(list) {
            _PluginManager_setup.call(this, list);
            for (const plugin of $plugins) {
                if (plugin.name && plugin.name.includes('/')) {
                    const baseName = plugin.name.substring(plugin.name.lastIndexOf('/') + 1);
                    if (Imported[baseName] === undefined) {
                        Imported[baseName] = true;
                    }
                    Imported[plugin.name] = true;
                }
            }
        };

        // PART 2: Patch PluginManager.parameters.
        // Reason: This allows plugins to retrieve their parameters correctly, whether they are
        // called by their base name ('MyPlugin') or their full path ('Folder/MyPlugin').
        const _PluginManager_parameters = PluginManager.parameters;
        PluginManager.parameters = function(name) {
            let params = _PluginManager_parameters.call(this, name);
            if (params && Object.keys(params).length === 0 && name && !name.includes('/')) {
                const plugin = $plugins.find(p => p.name.endsWith('/' + name));
                if (plugin) {
                    params = _PluginManager_parameters.call(this, plugin.name);
                }
            } else if (params && Object.keys(params).length === 0 && name && name.includes('/')) {
                const baseName = name.substring(name.lastIndexOf('/') + 1);
                const plugin = $plugins.find(p => p.name === baseName);
    ...
    // --- END OF SUBFOLDER PATCH ---

//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Fossil Core Engine (MV Command Handler)
//=============================================================================
// Date: 2025/08/11
// Reason: A definitive, robust handler for legacy MV plugin commands. This
// version incorporates an advanced argument parser that handles quoted strings,
// along with professional-grade error handling and conditional logging to ensure
// maximum compatibility and stability under all conditions.
//-----------------------------------------------------------------------------
PluginManager.registerCommand('FOSSIL', 'useOldPlugin', function(args) {
    // The 'this' context is the Game_Interpreter instance executing the command.
    const oldPluginCommand = String(args.OldPluginCommand || '').trim();
    if (!oldPluginCommand) {
        return;
    }

    // Advanced argument parser that correctly handles single and double-quoted strings.
    const commandParts = oldPluginCommand.match(/[^\s"']+|"([^"]*)"|'([^']*)'/g) || [];
    const command = commandParts.shift() || '';
    const cleanArgs = commandParts.map(arg => arg.replace(/^['"]|['"]$/g, ''));
    ...
    try {
        // Temporarily replace the interpreter's params, as many MV plugins read them directly.
        this._params = cleanArgs;

        // FINAL SAFEGUARD: Before calling, ensure `pluginCommand` actually exists and is a function.
        // This prevents crashes from rare cases of a corrupted interpreter state.
        if (typeof this.pluginCommand === "function") {
            this.pluginCommand(command, cleanArgs);
        }

    } catch (e) {
        // CONDITIONAL LOGGING: Errors are only logged to the console during a playtest.
        // This prevents flooding the console in a released game, keeping logs clean for the end-user.
        if (Utils.isOptionValid('test')) {
            console.error(`FOSSIL: Error executing MV plugin command "${oldPluginCommand}".`);
            console.error(`> Command: ${command} | Arguments:`, cleanArgs);
            console.error(`> Captured Error:`, e);
        }
        
    } finally {
        // CRITICAL: Always restore the interpreter's original `_params` state, even if an error occurred.
        this._params = originalParams;
    }
});
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Fossil Core Engine (MV Command Handler)
//=============================================================================

//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Fossil Core Engine
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Core Engine (Sprite_Enemy State Icons)
//=============================================================================
// Date: 2025/08/10
// Reason: Fixes state icon visibility and scaling issues on enemy sprites.
// This patch ensures icons are only visible when a battler has active states
// and prevents the icons from being stretched or shrunk along with the enemy sprite.
//-----------------------------------------------------------------------------
    // Alias the original method to extend its functionality without replacing it.
    var _Sprite_Enemy_updateBitmap = Sprite_Enemy.prototype.updateBitmap;

    Sprite_Enemy.prototype.updateBitmap = function() {
        // First, execute the original logic to ensure the bitmap is loaded correctly.
        _Sprite_Enemy_updateBitmap.call(this);

        // Find the state icon sprite by its class instance, which is the most reliable approach.
        const iconSprite = this.children.find(function(child) {
            return child instanceof Sprite_StateIcon;
        });

        // Exit early if the icon sprite or the enemy data is not available.
        if (!iconSprite || !this._enemy) {
            return;
        }
        
        // Determine if there are any active state icons to display.
        const hasIcons = this._enemy.stateIcons().length > 0;
        
        // The icon sprite should only be visible if there are states to show.
        iconSprite.visible = hasIcons;
        
        if (hasIcons) {
            // Reason: Counter-scale the icon to prevent it from being distorted if the parent
            // enemy sprite is scaled. This maintains the icon's original size and aspect ratio.
            // The check for non-zero scale avoids potential division-by-zero errors.
            if (this.scale.x !== 0 && this.scale.y !== 0) {
                iconSprite.scale.x = 1 / this.scale.x;
                iconSprite.scale.y = 1 / this.scale.y;
            }
        }
    };
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Core Engine (Sprite_Enemy State Icons)
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Core Engine (MV Animation Compatibility)
//=============================================================================
// Date: 2025/08/10
// Reason: Optimizes the handling of MV-style animations in RPG Maker MZ.
// It pre-processes `$dataAnimations` on load to flag each animation type (MV vs. MZ)
// and ensures compatibility. This avoids repeated, expensive checks during gameplay,
// improving performance and stability.
//-----------------------------------------------------------------------------
    // Alias the original method for safe extension.
    var _DataManager_onLoad = DataManager.onLoad;
    DataManager.onLoad = function(object) {
        // Call the original logic first.
        _DataManager_onLoad.apply(this, arguments);

        // Only process the animations data file ($dataAnimations).
        if (object === $dataAnimations) {
            for (var i = 1; i < object.length; i++) {
                var anim = object[i];
                if (!anim) continue;

                // An animation is identified as MV-style if it has a non-empty 'frames' array.
                var isMV = Array.isArray(anim.frames) && anim.frames.length > 0;
                
                if (isMV) {
                    // It's an MV animation. Prepare it for the MZ environment.
                    
                    // Ensure the 'timings' property exists, as it's expected by MV animation players.
                    if (!Array.isArray(anim.timings)) {
                        anim.timings = [];
                    }

                    // Add empty arrays for MZ-specific properties as a safeguard to prevent errors
                    // if other code attempts to access them.
                    if (!Array.isArray(anim.soundTimings)) {
                        anim.soundTimings = [];
                    }
                    if (!Array.isArray(anim.flashTimings)) {
                        anim.flashTimings = [];
                    }

                    // Create a flag for a fast and efficient check later.
                    anim._isMVAnimation = true;
                } else {
                    // This is an MZ (Effekseer) animation. Flag it as such for consistency.
                    anim._isMVAnimation = false;
                }
            }
        }
    };

    // Override isMVAnimation to use our pre-calculated flag.
    // This is significantly more performant than checking for 'animation.frames' on every call.
    Spriteset_Base.prototype.isMVAnimation = function(animation) {
        if (animation && typeof animation._isMVAnimation !== "undefined") {
            return animation._isMVAnimation;
        }
        // Provide a fallback in case the animation was not processed by our patch.
        return !!(animation && animation.frames);
    };
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Core Engine (MV Animation Compatibility)
//=============================================================================
	...
	*/
	...
	Window_Base.prototype.initialize = function(rect) {
		...
		if (arguments[0].constructor.name=='Rectangle' || 
        (rect && typeof rect.x === "number" && typeof rect.y === "number" &&
         typeof rect.width === "number" && typeof rect.height === "number")) // if our first argument is a rectangle this is MZ code
		{
	...
	Window_EquipSlot.prototype.initialize = function(rect) {
		...
		if (arguments.length>0 && arguments[0].constructor.name=='Rectangle' || 
        (rect && typeof rect.x === "number" && typeof rect.y === "number" &&
         typeof rect.width === "number" && typeof rect.height === "number")) // if our first argument is a rectangle this is MZ code
		{
	...
	Window_EquipItem.prototype.initialize = function(rect) {
		...
		if (arguments.length && arguments[0].constructor.name=='Rectangle' || 
        (rect && typeof rect.x === "number" && typeof rect.y === "number" &&
         typeof rect.width === "number" && typeof rect.height === "number")) // if our first argument is a rectangle this is MZ code
		{
	...
	Window_Help.prototype.initialize = function(rect) {
		...
		if ((arguments[0]!==undefined) && (arguments[0].constructor.name=='Rectangle' || 
        (rect && typeof rect.x === "number" && typeof rect.y === "number" &&
         typeof rect.width === "number" && typeof rect.height === "number"))) // if our first argument is a rectangle this is MZ code
		{
	...
	Window_ShopCommand.prototype.initialize = function(rect) {
		...
		if((typeof(arguments[0])!='undefined' ) && arguments[0].constructor.name=='Rectangle' || 
        (rect && typeof rect.x === "number" && typeof rect.y === "number" &&
         typeof rect.width === "number" && typeof rect.height === "number")) // if our first argument is a rectangle this is MZ code
		{
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Core Engine (Safe Area Scaler)
//=============================================================================
// Date: 2025/08/13
// Reason: A universal UI scaling utility that automatically adjusts the layout
// of all windows in a scene to fit within a 'safe area'. This prevents UI
// elements from being obscured by on-screen buttons (e.g., in mobile deployments).
// This version includes a corrected safeguard for smaller layouts.
//-----------------------------------------------------------------------------
(function() {

    // --- CONFIGURATION ---
    // Adjust these values to add a small margin (padding) inside the safe area.
    const PADDING = {
        TOP: 0,      // Pixels of padding at the top of the safe area.
        BOTTOM: 0,   // Pixels at the bottom. (1 is a good default value).
        LEFT: 0,     // Pixels on the left.
        RIGHT: 0     // Pixels on the right.
    };
    // -------------------

    // List of scenes that should NOT be adjusted.
    const EXCLUDED_SCENES = [
        // "Scene_Save"
    ];

    // List of windows that should NOT be adjusted.
    const EXCLUDED_WINDOWS = [
        // "Window_Help"
    ];

    /**
     * Defines the rectangle for the screen's safe area, applying the configurable padding.
     */
    function safeAreaRect() {
        const buttonHeight = SceneManager._scene?.buttonAreaHeight ? SceneManager._scene.buttonAreaHeight() : 0;
        const x = PADDING.LEFT;
        const y = buttonHeight + PADDING.TOP;
        const w = Graphics.boxWidth - (PADDING.LEFT + PADDING.RIGHT);
        const h = Graphics.boxHeight - (buttonHeight + PADDING.TOP + PADDING.BOTTOM);
        return new Rectangle(x, y, w, h);
    }

    /**
     * Recursively finds all Window instances within a container.
     */
    function findAllWindows(container) {
        let windows = [];
        if (!container || !container.children) return windows;
        ...
        for (const child of container.children) {
            ...
            if (child.children && child.children.length > 0) {
                windows = windows.concat(findAllWindows(child));
            }
        }
        return windows;
    }
    ...
    /**
     * Transforms the windows of a scene to fit perfectly within the safe area.
     */
    function transformWindowsToSafeArea(scene) {
        if (EXCLUDED_SCENES.includes(scene.constructor.name)) return;

        let windows = findAllWindows(scene)
            .filter(w => !EXCLUDED_WINDOWS.includes(w.constructor.name));
        ...
        // [CORRECTED SAFEGUARD CLAUSE]
        // Reason: If the window group's size is already smaller than or equal to the safe
        // area, no action is taken. This protects smaller, centered layouts from being
        // unnecessarily stretched to fill the entire screen.
        if (groupWidth <= area.width && groupHeight <= area.height) {
            return;
        }
        
        const scaleX = area.width / groupWidth;
        const scaleY = area.height / groupHeight;
        const offsetX = area.x - minX * scaleX;
        const offsetY = area.y - minY * scaleY;

        // Reason: The transformation is applied to each window using Math.round()
        // to ensure all final values are integers. This prevents 1-pixel
        // positioning and sizing errors that can occur from floating-point calculations.
        for (const win of windows) {
            win.x = Math.round(win.x * scaleX + offsetX);
            win.y = Math.round(win.y * scaleY + offsetY);
            win.width = Math.round(win.width * scaleX);
            win.height = Math.round(win.height * scaleY);

            if (typeof win.createContents === "function") {
                win.createContents();
            }
            if (typeof win.refresh === "function") {
                try { win.refresh(); } catch (e) { console.error("Error refreshing window:", e); }
            }
        }
    }
    
    // Reason: Overrides the `start` method of all scenes to execute our scaling logic
    // after the scene has been initialized but before it is rendered.
    const _Scene_Base_start = Scene_Base.prototype.start;
    Scene_Base.prototype.start = function() {
        _Scene_Base_start.call(this);
        
        // Reason: The transformation is applied to all scenes except for the boot and title
        // scenes, which typically have fixed, deliberate layouts that should not be altered.
        if (!(this instanceof Scene_Boot) && !(this instanceof Scene_Title)) {
            transformWindowsToSafeArea(this);
        }
    };

})();
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Core Engine (Safe Area Scaler)
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for Fossil Core Engine (Dynamic Fixes)
//=============================================================================
// Date: 2025/08/10
// Reason: Defines the core dynamic patching system for FOSSIL. This allows
// other patches to be scheduled to load before or after specific target
// plugins, with full support for plugins organized in subfolders.
//-----------------------------------------------------------------------------
fossilDynamicFixes = function() {
    Fossil.preList = [];
    Fossil.postList = [];

    // --- `loadPreFix` and `loadPostFix` Optimized for Subfolder Support ---
    
    // A helper function that finds a plugin's loaded instance, whether it's
    // referenced by its base name (e.g., 'YEP_CoreEngine') or its full path
    // (e.g., 'Yanfly/YEP_CoreEngine'). It returns the earliest and latest
    // matches found in the plugin load order.
    const findPluginInList = function(targetPlugins) {
        if (typeof targetPlugins === 'string') {
            targetPlugins = [targetPlugins];
        }

        let latestMatch = { name: null, index: -1 };
        let earliestMatch = { name: null, index: Infinity };

        for (let i = 0; i < Fossil.pluginNameList.length; i++) {
            const loadedPluginName = Fossil.pluginNameList[i];
            for (const targetName of targetPlugins) {
                if (loadedPluginName === targetName || loadedPluginName.endsWith('/' + targetName)) {
                    if (i < earliestMatch.index) {
                        earliestMatch = { name: loadedPluginName, index: i };
                    }
                    if (i > latestMatch.index) {
                        latestMatch = { name: loadedPluginName, index: i };
                    }
                }
            }
        }
    ...
    // Reason: Schedules a function to be executed immediately before the *first*
    // instance of a target plugin is loaded. This is used for patches that
    // must run before the target plugin initializes itself.
    Fossil.loadPreFix = function(targetPlugins, newFunction) {
        if (!targetPlugins || !newFunction) {
          console.log('Missing a function or targets when adding a pre plugin fix!')
          return;}
        const result = findPluginInList(targetPlugins);
        if (result.earliest.name) {
            Fossil.preList.push({ pluginName: result.earliest.name, preFunction: newFunction });
        }
    };
    
    // Reason: Schedules a function to be executed immediately after the *last*
    // instance of a target plugin has been loaded. This is used for patches
    // that modify or extend the target plugin after it has been fully initialized.
    Fossil.loadPostFix = function(targetPlugins, newFunction) {
        ...
        if (result.latest.name) {
            Fossil.postList.push({ pluginName: result.latest.name, postFunction: newFunction });
        }
    };
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for Fossil Core Engine (Dynamic Fixes)
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for MOG_LMBS
//=============================================================================
// Date: 2025/08/10
// Reason: A comprehensive compatibility patch for MOG_LMBS. It resolves issues
// with animation rendering (both MV and MZ), prevents crashes from
// uninitialized data, ensures correct behavior with other plugins like
// MOG_BattleCamera and YEP_SkillCore, and fixes various UI and gameplay logic bugs.
//-----------------------------------------------------------------------------
Fossil.loadPostFix('MOG_LMBS', function() {
(() => {

  // Reason: Ensures MV-style animations render above other battle elements by
  // setting a high z-index.
  if (typeof Sprite_AnimationMV !== 'undefined') {
    const _initMembersMV = Sprite_AnimationMV.prototype.initMembers;
    Sprite_AnimationMV.prototype.initMembers = function() {
      _initMembersMV.call(this);
      this._zIndex = 9999;
    };
  }

  // Reason: Prevents a crash when `equips()` is called on an actor
  // with no equipment slots initialized.
  if (typeof Game_Actor !== 'undefined') {
    const _equips = Game_Actor.prototype.equips;
    Game_Actor.prototype.equips = function() {
      return this._equips ? _equips.call(this) : [];
    };
  }

  // Reason: Overrides `allTraits` to prevent errors if a trait object
  // (e.g., actor, class, state) is null or lacks a `traits` property.
  if (typeof Game_BattlerBase !== 'undefined') {
    Game_BattlerBase.prototype.allTraits = function() {
      return this.traitObjects().reduce((r, obj) => (obj?.traits) ? r.concat(obj.traits) : r, []);
    };
  }

  // Reason: Adds `isAnimationRequested` and `shiftAnimation` methods if they
  // don't exist, providing backward compatibility for plugins expecting these RMMV methods.
  if (typeof Game_Battler !== 'undefined') {
    if (!Game_Battler.prototype.isAnimationRequested) {
      Game_Battler.prototype.isAnimationRequested = function() {
        return !!this._animationId;
      };
    }
    if (!Game_Battler.prototype.shiftAnimation) {
      Game_Battler.prototype.shiftAnimation = function() {
        const data = {
          animationId: this._animationId,
          mirror: this._animationMirror,
          delay: this._animationDelay
        };
        this._animationId = this._animationMirror = this._animationDelay = 0;
        return data;
      };
    }
  }
  ...
  if (typeof Sprite_Animation !== 'undefined') {
    // Reason: Guarantees that animation timing arrays (`soundTimings`, `flashTimings`)
    // exist before setup, preventing crashes with incomplete animation data.
    const _setup = Sprite_Animation.prototype.setup;
    Sprite_Animation.prototype.setup = function(targets, animation, mirror, delay, previous) {
      if (animation) {
        ...
        animation.timing ??= animation.timings || [];
      }
      _setup.call(this, targets, animation, mirror, delay, previous);
    };

    // Reason: Ensures the animation's rotation property is a valid object with
    // x, y, z numbers, preventing potential rendering errors from bad data.
    const _updateGeom = Sprite_Animation.prototype.updateEffectGeometry;
    Sprite_Animation.prototype.updateEffectGeometry = function() {
      if (!this._animation) return;
      const rot = this._animation.rotation;
      if (!rot || typeof rot.x !== 'number' || typeof rot.y !== 'number' || typeof rot.z !== 'number') {
        this._animation.rotation = { x: 0, y: 0, z: 0 };
      }
      _updateGeom.call(this);
    };

    // Reason: Adds a `remove` method if it's missing, ensuring the sprite can be
    // safely removed from its parent container.
    if (!Sprite_Animation.prototype.remove) {
  ...
  // Reason: Extends skill initialization to read a custom animation ID from skill/weapon
  // notetags (`LMBS Animation ID: X`), allowing for custom cast animations.
  if (typeof LMBS_Skill !== 'undefined') {
    const _LMBS_Skill_initialize = LMBS_Skill.prototype.initialize;
    LMBS_Skill.prototype.initialize = function(battler, skill, sprite, target, waitime) {
      _LMBS_Skill_initialize.call(this, battler, skill, sprite, target, waitime);

      // Note: Adjust if your basic attack skill uses a different ID.
      const ATTACK_SKILL_ID = 1;

      const playAnimFromNote = (note) => {
        const m = (note || '').match(/lmbs animation id\s*:\s*(\d+)/i);
        if (m) {
          const id = Number(m[1]) || 0;
          if (id > 0) {
            // Store the ID for compatibility with MOG_LMBS and RMMZ internals.
            this._animationID = id;
            if (Array.isArray(this._itemData)) this._itemData[9] = id;

            // Play the animation on the user immediately to maintain expected behavior.
            const user = this._user;
            if (user) {
              if (typeof user.startAnimation === 'function') {
                user.startAnimation(id, this._direction === 2, 0);
              } else if ($gameTemp?.requestAnimation) {
                $gameTemp.requestAnimation([user], id, this._direction === 2);
              }
            }
          }
        }
      ...
      // Check the skill's notetag first.
      playAnimFromNote(this._item?.note || '');

      // If it's a basic attack, also check the user's equipped weapon notetag.
      if (this._item?.id === ATTACK_SKILL_ID) {
        const weapon = this._user?.equips?.()[0];
        playAnimFromNote(weapon?.note || '');
      }
    };

    // Reason: Ensures the default impact animation (from the database) is played
    // on the target battler. A flag prevents the animation from repeating.
    const _LMBS_updateEffect = LMBS_Skill.prototype.updateBattlerEffect;
    LMBS_Skill.prototype.updateBattlerEffect = function(battler, index) {
      _LMBS_updateEffect.call(this, battler, index);

      if (!battler._lmbsImpactAnimPlayed) {
        const aniId = this._item?.animationId || 0;
        if (aniId > 0) {
          if (typeof battler.startAnimation === 'function') {
            battler.startAnimation(aniId, this._direction === 2, 0);
          } else if ($gameTemp?.requestAnimation) {
            $gameTemp.requestAnimation([battler], aniId, this._direction === 2);
          }
        }
        battler._lmbsImpactAnimPlayed = true;
      }
    };
  }

  // Reason: Hooks into the main battle update loop to call the LMBS physics update
  // for each battler and then repositions their sprites accordingly.
  if (typeof Spriteset_Battle !== 'undefined') {
    const _updateBattle = Spriteset_Battle.prototype.update;
    Spriteset_Battle.prototype.update = function() {
      _updateBattle.call(this);
      [...$gameParty.battleMembers(), ...$gameTroop.members()].forEach(b => b.lmbsUpdatePhysics?.());
      this.lmbsUpdateBattlerPositions();
    };

    Spriteset_Battle.prototype.lmbsUpdateBattlerPositions = function() {
      const updatePos = (sprite, battler) => {
        if (!battler) return;
        if (battler._lmbs_X !== undefined) sprite.x = battler._lmbs_X;
        if (battler._lmbs_Y !== undefined) {
          sprite.y = battler._lmbs_Y - (battler.lmbsHeight?.() / 2 || 0);
        }
      };
      this._actorSprites.forEach(s => updatePos(s, s._actor));
      this._enemySprites.forEach(s => updatePos(s, s._enemy));
    };
  }

  // Reason: Overrides the default animation positioning logic to align MZ/Effekseer
  // animations with the battler's dynamic LMBS coordinates (_lmbs_X, _lmbs_Y)
  // instead of the static sprite position.
  if (typeof Sprite_Animation !== 'undefined') {
    const _origTargetPos = Sprite_Animation.prototype.targetSpritePosition;
    Sprite_Animation.prototype.targetSpritePosition = function(sprite) {
      if (SceneManager._scene instanceof Scene_Battle) {
        const spriteset = SceneManager._scene._spriteset;
        const bf = spriteset?._battleField;
        if (!spriteset || !bf) return _origTargetPos.call(this, sprite);

        let battler = null;
        if (sprite._battler?._lmbs_X !== undefined) {
          battler = sprite._battler;
        } else if (typeof LMBS_Skill !== 'undefined' && sprite instanceof LMBS_Skill && sprite._user) {
          battler = sprite._user;
        }

        if (battler) {
          bf.updateTransform();
          const origin = bf.worldTransform;
          const pt = new Point();
          pt.x = origin.tx + battler._lmbs_X;
          pt.y = origin.ty + battler._lmbs_Y - (battler.lmbsHeight?.() / 2 || 0);
          if (this._animation?.alignBottom) pt.y += (battler.lmbsHeight?.() / 2 || 0);
          return pt;
        }
      }
      return _origTargetPos.call(this, sprite);
    };
  }

  // Reason: Intercepts adding new children to PIXI containers during a battle.
  // If an RMMZ Effekseer animation is added to the low-priority `_effectsContainer`,
  // it is redirected to the main spriteset and assigned a high `zIndex`. This
  // fixes a common layering issue, ensuring modern animations render above battlers.
  (function() {
    if (typeof PIXI === 'undefined' || typeof SceneManager === 'undefined') return;
    const _PIXI_addChild = PIXI.Container.prototype.addChild;
    PIXI.Container.prototype.addChild = function(child) {
      if (SceneManager._scene instanceof Scene_Battle) {
        const spriteset = SceneManager._scene._spriteset;
        if (spriteset && this === spriteset._effectsContainer &&
            child instanceof Sprite_Animation && !(child instanceof Sprite_AnimationMV)) {
          // Instead of adding to _effectsContainer, move it to the main spriteset
          // with a high zIndex for visual priority, avoiding global modifications.
          spriteset.addChild(child);
          child._zIndex = 9999;
          return child;
        }
      }
      return _PIXI_addChild.call(this, child);
    };
  })();

  // Compatibility Note: This patch modifies `PIXI.Container.prototype.addChild`.
  // To ensure it functions correctly, place this plugin *after* any other
  // plugins that also modify this method or related animation methods.
})();


(() => {

    // Reason: Adjusts the menu window height to correctly fit its content (2 lines).
    LmbsMenuWindow.prototype.windowHeight = function() {
        return this.fittingHeight(2);
    };

    // Reason: Optimizes the drawing of command icons by using a pre-calculated map,
    // avoiding redundant logic inside the loop.
    LMBSwindowSkillEquip.prototype.lmbsDrawCom = function(index) {
        if (!this._comImg) return;
        const bitmap = this._comImg;
        const cw = bitmap.width / 10;
        const ch = bitmap.height;
        const rect = this.itemRect(index);
        const xOffset = rect.x;
        const yOffset = rect.y + (rect.height - ch) / 2;
        ...
        (iconMap[index] || []).forEach((frame, i) => {
            this.contents.blt(bitmap, cw * frame, 0, cw, ch, xOffset + (i * cw), yOffset);
        });
    };

    // Reason: Refines the skill drawing logic for better alignment and readability,
    // properly positioning the icon, name, and cost.
    LMBSwindowSkillList.prototype.drawItem = function(index) {
        ...
        const iconX = rect.x + leftMargin;
        const iconY = rect.y + (rect.height - iconH) / 2;
        this.drawIcon(skill.iconIndex, iconX, iconY);

        const textX = iconX + iconW + 4;
        const costWidth = this.costWidth();
        const textWidth = rect.width - iconW - 4 - leftMargin - rightMargin;

        this.drawText(skill.name, textX, rect.y, textWidth);
        this.drawSkillCost(skill, rect.x + rect.width - costWidth - rightMargin, rect.y, costWidth);
    };

    // Reason: Patches cursor and camera focus logic for compatibility with MOG_BattleCamera.
    // This ensures cursors and focus remain locked on their targets as the camera moves.
    if (Imported?.MOG_BattleCamera) {
        const getCameraOffsets = (spriteset) => {
            if (spriteset?._baseSprite) {
                return [spriteset._baseSprite.x, spriteset._baseSprite.y];
            }
            return [spriteset?._battleField?.x || 0, spriteset?._battleField?.y || 0];
        };

        const _lmbsUpdateCursorB = Spriteset_Battle.prototype.lmbsUpdateCursorB;
        Spriteset_Battle.prototype.lmbsUpdateCursorB = function() {
            if (this._lmbsCursorP.opacity > 0 && BattleManager._lmbsActor) {
                this.lmbsUpdateCrF();
                const [offX, offY] = getCameraOffsets(this);
                const x = BattleManager._lmbsActor._lmbs_X + Moghunter.lmbsCursorPlayerX;
                const y = BattleManager._lmbsActor._lmbs_Y
                        - BattleManager._lmbsActor.lmbsBodyHeight() - 24
                        + Moghunter.lmbsCursorPlayerY
                        + (this._lmbsCursorFl?.[1] || 0);
                this._lmbsCursorP.x = offX + x;
                this._lmbsCursorP.y = offY + y;
            }
            if (($gameTemp?._lmbsPlayerCursor?.[0] || 0) > 0) {
        ...
        const _lmbsUpdateCursor = Spriteset_Battle.prototype.lmbsUpdateCursor;
        Spriteset_Battle.prototype.lmbsUpdateCursor = function() {
            if (this.lmbsCanUpdateCursor()) {
                const target = $gameTemp?._lmbsTarget?.[0];
                if (target) {
                    const y = (target.lmbsBodyHeight() > 200) ?
                        target._lmbs_Y - (target.lmbsBodyHeight() / 2) :
                        target._lmbs_Y - target.lmbsBodyHeight();
                    this._lmbsCursorN[0] = this.lmbsCursorMove(this._lmbsCursorN[0], target._lmbs_X, 5);
                    this._lmbsCursorN[1] = this.lmbsCursorMove(this._lmbsCursorN[1], y, 5);
                    this._lmbsCursor.opacity += 25;
                    $gameTemp._lmbsFocus[0] = target._lmbs_X;
                    $gameTemp._lmbsFocus[1] = target._lmbs_Y;
                }
            } else {
                this._lmbsCursor.opacity -= 35;
            }
            const [offX, offY] = getCameraOffsets(this);
            this._lmbsCursor.x = offX + this._lmbsCursorN[0];
            this._lmbsCursor.y = offY + this._lmbsCursorN[1];
            ...
            if ($gameTemp?._lmbsTarget?.[0] &&
                this._lmbsCursorNameText !== $gameTemp._lmbsTarget[0].name()) {
                this.lmbsRefreshCursorName();
            }
        };

        const _updateFocus = Spriteset_Battle.prototype.updateFocus;
        Spriteset_Battle.prototype.updateFocus = function() {
            if ($gameSystem?._lmbsData?.[0]) {
                this._cam_X = $gameTemp?._lmbsFocus?.[0] || 0;
                this._cam_Y = ($gameTemp?._lmbsFocus?.[1] || 0) - 48;
                return;
            }
            _updateFocus.call(this);
        };

        const _battleCamSetup = Spriteset_Battle.prototype.battleCamSetup;
        Spriteset_Battle.prototype.battleCamSetup = function() {
            _battleCamSetup.call(this);
...
// Reason: Reconstructs the LMBS equip scene to feature dynamic window sizing
// and to correctly instantiate and manage the help window, fixing layout issues
// and improving adaptability to different screen resolutions.
//-----------------------------------------------------------------------------
Scene_LMBSEquip.prototype.create = function() {
    ...
    this._lmbsWindowActor = new Window_SkillStatus(0, Graphics.boxHeight - 180, Graphics.boxWidth, 180);
    this._lmbsWindowActor.setActor(this._actor);
    this.addWindow(this._lmbsWindowActor);

    const wy = this._helpWindow.height;
    const wh = Graphics.boxHeight - wy - this._lmbsWindowActor.height;

    this._lmbsWindowSkillEquip = new LMBSwindowSkillEquip(0, wy, Graphics.boxWidth / 2, wh);
    this._lmbsWindowSkillEquip.setHelpWindow(this._helpWindow);
    this.addWindow(this._lmbsWindowSkillEquip);
    ...
    this._lmbsWindowSkillList = new LMBSwindowSkillList(this._lmbsWindowSkillEquip.width, wy, Graphics.boxWidth / 2, wh);
    this._lmbsWindowSkillList.setHelpWindow(this._helpWindow);
    this.addWindow(this._lmbsWindowSkillList);
...
// Reason: Fixes a compatibility issue with YEP_SkillCore. This patch overrides
// the LMBS item drawing methods to correctly call Yanfly's skill cost drawing
// function, preventing visual errors and crashes that occur when Moghunter's
// original drawing logic is used alongside Yanfly's system.
//-----------------------------------------------------------------------------
if (Imported && Imported.YEP_SkillCore) {

    // --- Left Window (Equipped Skills) ---
    LMBSwindowSkillEquip.prototype.drawItem = function(index) {
        const skill = this._data[index];
        const rect = this.itemRect(index);
        this.contents.clearRect(rect.x, rect.y, rect.width, rect.height);
        
        // First, draw Moghunter's custom command icons (attack, defend, etc.).
        this.lmbsDrawCom(index);
        
        if (skill) {
            // Draw the skill name, reserving 150px on the right for skill costs.
            const nameWidth = rect.width - 38 - 150;
            this.drawItemName(skill, rect.x + 38, rect.y, nameWidth);
            
            // Reason: Call Yanfly's cost drawing function. Providing the full item rect width
            // allows Yanfly's logic to handle right-alignment correctly.
            this.drawSkillCost(skill, rect.x, rect.y, rect.width);
        }
    };

    // --- Right Window (Skills to Equip) ---
    LMBSwindowSkillList.prototype.drawItem = function(index) {
        const skill = this._data[index];
        if (skill && skill.name) {
            const rect = this.itemRect(index);
            ...
            // Draw the skill name, leaving space on the right for the cost.
            const nameWidth = rect.width - costWidth;
            this.drawItemName(skill, rect.x, rect.y, nameWidth);

            // Reason: Call Yanfly's cost drawing function, giving it the full width
            // of the item rect to correctly handle its own right-alignment.
            this.drawSkillCost(skill, rect.x, rect.y, rect.width);
        } else if (skill) { // For the "empty" or unnamed item slot.
            const rect = this.itemRect(index);
            this.contents.clearRect(rect.x, rect.y, rect.width, rect.height);
...
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for MOG_LMBS
//=============================================================================

//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for MOG_DmgPopupEffects
//=============================================================================
// Date: 2025/08/10
// Reason: Provides essential compatibility shims to make the MV plugin
// `MOG_DmgPopupEffects` work in RPG Maker MZ. It polyfills missing properties
// on `Sprite_Damage` and adapts a function signature mismatch.
//-----------------------------------------------------------------------------

Fossil.loadPostFix('MOG_DmgPopupEffects', function() {
    

    // --- 1. Essential Polyfills for Sprite_Damage ---
    // Reason: RMMZ's Sprite_Damage lacks certain properties and methods from RMMV that
    // Moghunter's code relies on. This section adds them back for compatibility.

    // Reason: Ensures the damage spritesheet (Damage.png) is loaded upon initialization.
    ...
    // Reason: Defines the width of a single digit on the spritesheet.
    if (!Sprite_Damage.prototype.digitWidth) {
        Sprite_Damage.prototype.digitWidth = function() {
            return this._damageBitmap ? this._damageBitmap.width / 10 : 0;
        };
    }

    // Reason: Defines the height of a single row on the spritesheet (MOG's version uses 8 rows).
    if (!Sprite_Damage.prototype.digitHeight) {
        Sprite_Damage.prototype.digitHeight = function() {
            return this._damageBitmap ? this._damageBitmap.height / 8 : 0;
        };
    }

    // --- 2. Signature Adapter for createDigits ---
    // Reason: The RMMZ engine calls createDigits with one argument (value), while
    // Moghunter's plugin expects two (baseRow, value). This wrapper adapts the
    // function call to bridge this incompatibility.

    const _mog_createDigits = Sprite_Damage.prototype.createDigits;
    Sprite_Damage.prototype.createDigits = function(baseRow, value) {
        // If only one argument is received, it's a call from the MZ core engine.
        if (arguments.length === 1) {
            value = baseRow;
            // Assume row 0 (standard HP damage) by default.
            baseRow = 0;
        }
        // Call Moghunter's original function with the corrected arguments.
        _mog_createDigits.call(this, baseRow, value);
    };

});
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for MOG_DmgPopupEffects
//=============================================================================
  
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for YEP_ItemCore
//=============================================================================
// Date: 2025/08/11
// Reason: Recreates the YEP_ItemCore scene layout for RPG Maker MZ.
// v1.3 - Adds an explicit onActorCancel method to prevent conflicts with other
// plugins (like ARPG_ItemShortcut) that might alias the same function.
//-----------------------------------------------------------------------------

Fossil.loadPostFix('YEP_ItemCore', function() {

    // Reason: This entire patch is conditional. It only runs if `Window_ItemStatus` exists.
    // This class is only defined by YEP_ItemCore when its scene modification feature is enabled.
    // This check prevents the patch from running and causing errors if the user has disabled
    // the feature in the plugin's parameters.
    if (typeof Window_ItemStatus === 'function') {

        if (Fossil.chattyOutput) {
            console.log('FOSSIL: YEP_ItemCore scene modification is active. Applying scene patch.');
        }
        ...
        Scene_Item.prototype.create = function() {
            ...
            if (this._itemWindow) {
                if (this._statusWindow) this._itemWindow.setStatusWindow(this._statusWindow);
                if (this._infoWindow) this._itemWindow.setInfoWindow(this._infoWindow);
            }
        };

        // --- Window Rectangle Definitions ---
        Scene_Item.prototype.helpWindowRect = function() { return new Rectangle(0, 0, Graphics.boxWidth, this.calcWindowHeight(2, false)); };
        Scene_Item.prototype.categoryWindowRect = function() { const hRect = this.helpWindowRect(); return new Rectangle(0, hRect.y + hRect.height, 240, this.calcWindowHeight(4, true)); };
        Scene_Item.prototype.statusWindowRect = function() { const cRect = this.categoryWindowRect(); return new Rectangle(cRect.x + cRect.width, cRect.y, Graphics.boxWidth - cRect.width, cRect.height); };
        Scene_Item.prototype.itemWindowRect = function() { const cRect = this.categoryWindowRect(); return new Rectangle(0, cRect.y + cRect.height, Graphics.boxWidth / 2, Graphics.boxHeight - (cRect.y + cRect.height)); };
        Scene_Item.prototype.infoWindowRect = function() { const iRect = this.itemWindowRect(); return new Rectangle(iRect.x + iRect.width, iRect.y, Graphics.boxWidth - iRect.width, iRect.height); };
        Scene_Item.prototype.actionWindowRect = function() { return this.itemWindowRect(); };

        // --- Window Creation ---
        Scene_Item.prototype.createCategoryWindow = function() { const rect = this.categoryWindowRect(); this._categoryWindow = new Window_ItemCategory(rect); this._commandWindow = this._categoryWindow; this._categoryWindow.setHelpWindow(this._helpWindow); this._categoryWindow.setHandler("ok", this.onCategoryOk.bind(this)); this._categoryWindow.setHandler("cancel", this.popScene.bind(this)); this.addWindow(this._categoryWindow); };
        Scene_Item.prototype.createStatusWindow = function() { const rect = this.statusWindowRect(); this._statusWindow = new Window_ItemStatus(rect.x, rect.y, rect.width, rect.height); this.addWindow(this._statusWindow); };
        Scene_Item.prototype.createItemWindow = function() { const rect = this.itemWindowRect(); this._itemWindow = new Window_ItemList(rect); this._itemWindow.setHelpWindow(this._helpWindow); this._itemWindow.setHandler("ok", this.onItemOk.bind(this)); this._itemWindow.setHandler("cancel", this.onItemCancel.bind(this)); this.addWindow(this._itemWindow); this._categoryWindow.setItemWindow(this._itemWindow); };
        Scene_Item.prototype.createInfoWindow = function() { const rect = this.infoWindowRect(); this._infoWindow = new Window_ItemInfo(rect.x, rect.y, rect.width, rect.height); this.addWindow(this._infoWindow); };
        Scene_Item.prototype.createActionWindow = function() { const rect = this.actionWindowRect(); this._itemActionWindow = new Window_ItemActionCommand(rect.x, rect.y); this._itemActionWindow.width = rect.width; this._itemActionWindow.height = rect.height; this._itemActionWindow.setHandler("use", this.onActionUse.bind(this)); this._itemActionWindow.setHandler("cancel", this.onActionCancel.bind(this)); this.addWindow(this._itemActionWindow); };

        // --- Scene Logic and Flow ---

        // Reason: Adds an explicit `onActorCancel` handler. This prevents conflicts with other plugins
        // (e.g., ARPG_ItemShortcut) that might alias the default scene's non-existent handler,
        // ensuring this scene returns focus to the item window correctly when cancelling actor selection.
        ...
        Scene_Item.prototype.onItemOk = function() { this._itemActionWindow.setItem(this.item()); };
        const _Scene_Item_onItemCancel = Scene_Item.prototype.onItemCancel;
        Scene_Item.prototype.onItemCancel = function() { _Scene_Item_onItemCancel.call(this); if (this._statusWindow) this._statusWindow.setItem(null); if (this._infoWindow) this._infoWindow.setItem(null); };
        Scene_Item.prototype.onActionUse = function() { this._itemActionWindow.hide(); this._itemActionWindow.deactivate(); $gameParty.setLastItem(this.item()); this.determineItem(); };
        Scene_Item.prototype.onActionCancel = function() { this._itemActionWindow.hide(); this._itemActionWindow.deactivate(); this._itemWindow.activate(); };
        Scene_Item.prototype.popScene = function() { $gameParty.members().forEach(function(member) { if (member) member.refresh(); }); Scene_Base.prototype.popScene.call(this); };

        // --- Adjustments to YEP Window Classes ---
        (function() {
            const _Window_ItemCategory_maxCols = Window_ItemCategory.prototype.maxCols;
            Window_ItemCategory.prototype.maxCols = function() { if (SceneManager._scene instanceof Scene_Item) return 1; return _Window_ItemCategory_maxCols.call(this); };
            const _Window_ItemList_maxCols = Window_ItemList.prototype.maxCols;
            Window_ItemList.prototype.maxCols = function() { if (SceneManager._scene instanceof Scene_Item) return 1; return _Window_ItemList_maxCols.call(this); };
        })();

        (function(){
            const _Window_ItemList_initialize = Window_ItemList.prototype.initialize;
            Window_ItemList.prototype.initialize = function(rect) { if (typeof rect === 'object' && rect.x !== undefined) { _Window_ItemList_initialize.call(this, rect.x, rect.y, rect.width, rect.height); } else { _Window_ItemList_initialize.apply(this, arguments); }};
        })();
    
    } else {
        if (Fossil.chattyOutput) {
            console.log('FOSSIL: YEP_ItemCore scene modification is disabled. Skipping scene patch.');
        }
    }
});

//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for YEP_ItemCore
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for YEP_StatusMenuCore
//=============================================================================
// Date: 2025/08/11
// Reason: A comprehensive patch that merges FOSSIL's original low-level
// compatibility layer for gauges with a full UI reconstruction to replicate
// the YEP_StatusMenuCore layout and functionality in RPG Maker MZ.
//-----------------------------------------------------------------------------
Fossil.loadPostFix('YEP_StatusMenuCore', function() {

    //=============================================================================
    // PART 1: LOW-LEVEL COMPATIBILITY (FROM THE ORIGINAL FOSSIL)
    // Reason: Prepares the base MZ classes so they can be used by Yanfly's logic.
    //=============================================================================

    // Reason: Extends Sprite_Gauge to understand YEP_StatusMenuCore's custom data types
    // (XP and parameters) for `currentValue`, `currentMaxValue`, and `label`.
    const _Fossil_YEP_SMC_Sprite_Gauge_currentValue = Sprite_Gauge.prototype.currentValue;
    Sprite_Gauge.prototype.currentValue = function() {
        if (typeof(this._statusType) === 'number') {
            if (SceneManager._scene && SceneManager._scene._infoWindow && SceneManager._scene._infoWindow._actor) {
                return SceneManager._scene._infoWindow._actor.param(this._statusType);
            }
        }
        if (this._battler) {
            switch (this._statusType) {
                case "xp": return Math.floor(100 * Window_StatusInfo.prototype.actorCurrentExpRate(this._battler));
                case "xp2": return Math.floor(100 * Window_StatusInfo.prototype.actorExpRate(this._battler));
            }
        }
        return _Fossil_YEP_SMC_Sprite_Gauge_currentValue.call(this);
    };

    const _Fossil_YEP_SMC_Sprite_Gauge_currentMaxValue = Sprite_Gauge.prototype.currentMaxValue;
    Sprite_Gauge.prototype.currentMaxValue = function() {
        if (typeof(this._statusType) === 'number') {
            if (SceneManager._scene && SceneManager._scene._infoWindow) {
                // For parameters, the max value is a pre-calculated largest param for normalization.
        ...
        if (this._battler) {
            switch (this._statusType) {
                case "xp":
                case "xp2":
                    // For XP, the value is a percentage, so the max is always 100.
                    return 100;
            }
        }
        return _Fossil_YEP_SMC_Sprite_Gauge_currentMaxValue.call(this);
    };

    const _Fossil_YEP_SMC_Sprite_Gauge_label = Sprite_Gauge.prototype.label;
    Sprite_Gauge.prototype.label = function() {
        if (typeof(this._statusType) === 'number') return ''; // Parameter labels are drawn elsewhere.
        switch (this._statusType) {
            case "xp":
            case "xp2":
                return TextManager.expA + '%';
        }
        return _Fossil_YEP_SMC_Sprite_Gauge_label.call(this);
    };

    // Reason: Polyfills Window_StatusInfo with methods from Window_StatusBase. This acts as a mixin,
    // giving the Yanfly window core drawing capabilities it needs to function in MZ but lacks by default.
    ...
    // Reason: Ensures the `_additionalSprites` container exists. This object is required
    // by FOSSIL's `drawGauge` function to store and manage the gauge sprites it creates.
    const _Fossil_YEP_SMC_Window_StatusInfo_initialize = Window_StatusInfo.prototype.initialize;
    Window_StatusInfo.prototype.initialize = function() {
        this._additionalSprites = {};
        _Fossil_YEP_SMC_Window_StatusInfo_initialize.apply(this, arguments);
    };

    //=============================================================================
    // PART 2: UI RECONSTRUCTION (FROM THE ENHANCED SHK PATCH)
    // Reason: Draws the windows and gauges according to Yanfly's original design.
    //=============================================================================
    ...
    Window_StatusInfo.prototype.drawParamGauge = function(dx, dy, dw, paramId) {
        ...
        // Reason: Dynamically constructs the parameter name to fetch colors from Yanfly's
        // parameters. This is safer and more performant than using eval().
        var paramName = 'ColorParam' + paramId + 'Gauge';
        var colorString = Yanfly.Param[paramName] || '0 0';
        var array = colorString.split(' ');
        var color1, color2;

        // Reason: Handles two color formats from Yanfly parameters: numeric color indices
        // (e.g., "17 18") and direct color strings (e.g., "blue").
        if (!isNaN(Number(array[0]))) {
            const colorIndex1 = Number(array[0]) || 0;
            const colorIndex2 = Number(array[1]) || colorIndex1;
            color1 = this.textColor(colorIndex1);
            color2 = this.textColor(colorIndex2);
        } else {
            color1 = array[0];
            color2 = array[1] || array[0];
        }
    ...
    // Reason: Overridden to prevent the default dark item background from being
// drawn in the main status info window. This function is intentionally left empty.
Window_StatusInfo.prototype.drawItemBackground = function(index) {
    // Do nothing.
};

// Reason: Overridden to prevent the default dark item background from being
// drawn in the skill status window. This function is intentionally left empty.
Window_SkillStatus.prototype.drawItemBackground = function(index) {
    // Do nothing.
};

    // Reason: Ensures proper cleanup of gauge sprites when the window is refreshed.
    // Calling `hideAdditionalSprites` removes any FOSSIL-managed gauge sprites from the
    // previous draw cycle, preventing visual artifacts like leftover gauges.
    const _Fossil_YEP_SMC_hideSprites_Window_StatusInfo_refresh = Window_StatusInfo.prototype.refresh;
    Window_StatusInfo.prototype.refresh = function() {
        this.hideAdditionalSprites();
        _Fossil_YEP_SMC_hideSprites_Window_StatusInfo_refresh.call(this);
    };

});
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for YEP_StatusMenuCore
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for YEP_ShopMenuCore
//=============================================================================
// Date: 2025/08/11
// Reason: A comprehensive patch that merges FOSSIL's original low-level
// compatibility fixes with a full UI reconstruction to replicate the classic
// YEP_ShopMenuCore layout and functionality in RPG Maker MZ.
//-----------------------------------------------------------------------------
Fossil.loadPostFix('YEP_ShopMenuCore', function() {

    //=============================================================================
    // PART 1: Initialization and Layout Fixes (From the Original FOSSIL)
    // Reason: These fixes are essential for low-level compatibility between
    // Yanfly's MV classes and the MZ engine's scene structure.
    //=============================================================================

    // Reason: YEP_ShopMenuCore's category window needs to be explicitly given the
    // dimensions of the default command window it replaces. This ensures correct
    // positioning and size.
    Window_ShopCategory.prototype.initialize = function() {
        const rect = new Rectangle(
            SceneManager._scene._commandWindow.x,
            SceneManager._scene._commandWindow.y,
            SceneManager._scene._commandWindow.width,
            SceneManager._scene._commandWindow.height
        );
        Window_ItemCategory.prototype.initialize.call(this, rect);
    };

    // Reason: In MV, shop goods were passed as a separate argument. This shim
    // captures that argument and assigns it to `this._shopGoods` to maintain
    // compatibility with the original plugin's logic.
    const _Fossil_ShopMenuCore_Window_ShopBuy_initialize = Window_ShopBuy.prototype.initialize;
    Window_ShopBuy.prototype.initialize = function() {
        this._shopGoods = this._shopGoods || arguments[3]; // Captures 'goods' from the MV constructor signature.
        _Fossil_ShopMenuCore_Window_ShopBuy_initialize.apply(this, arguments);
    };

    // Reason: Overrides the help window's vertical position. YEP_ShopMenuCore's
    // layout is designed with the expectation that the help window is always at `y = 0`.
    const _Fossil_ShopMenuCore_Scene_Shop_helpAreaTop = Scene_Shop.prototype.helpAreaTop;
    Scene_Shop.prototype.helpAreaTop = function() {
        if (this.constructor.name === "Scene_Shop") return 0;
        return _Fossil_ShopMenuCore_Scene_Shop_helpAreaTop.call(this);
    };

    //=============================================================================
    // PART 2: Scene and Window Reconstruction (SHK's Patch)
    // Reason: This section rebuilds the interface to match Yanfly's classic shop layout.
    //=============================================================================
    (function() {
        ...
        // --- 2. Window_ShopNumber Logic ---

        Window_ShopNumber.prototype.refresh = function() {
            this.contents.clear();
            this.resetFontSettings();
            this.drawItemName(this._item, 0, this.itemNameY(), this.innerWidth);
        ...
        Window_ShopNumber.prototype.totalPriceY = function() {
            return this.itemNameY() + Math.floor(this.lineHeight() * 2.5);
        };

        Window_ShopNumber.prototype.drawTotalPrice = function() {
            const width = this.innerWidth - this.padding * 2;
            const y = this.totalPriceY();
            this.drawHorzLine(y - this.lineHeight() / 2);
            this.drawTotalCurrency(width, y);
            this.drawTotalCost(width, y + this.lineHeight());
            this.drawHorzLine(y + this.lineHeight() * 2 - this.lineHeight() / 2);
            this.drawTotalAfter(width, y + this.lineHeight() * 2);
        };

        Window_ShopNumber.prototype.drawHorzLine = function(y) {
            this.contents.paintOpacity = 48;
            this.contents.fillRect(0, y, this.innerWidth, 2, this.normalColor());
            this.contents.paintOpacity = 255;
        };

        Window_ShopNumber.prototype.drawTotalCurrency = function(width, y) {
            const value = Yanfly.Util.toGroup(this.getTotalCurrency());
            this.drawCurrencyValue(value, this._currencyUnit, 0, y, width);
        };

        Window_ShopNumber.prototype.getTotalCurrency = function() {
            return (this._currencyUnit === TextManager.currencyUnit) ? $gameParty.gold() : 0;
        };
        
        Window_ShopNumber.prototype.drawTotalCost = function(width, y) {
            let value = this._price * this._number;
            if (!this.isSelling()) {
                value *= -1;
            }
            const s = value > 0 ? "+" : "";
            this.drawCurrencyValue(s + Yanfly.Util.toGroup(value), this._currencyUnit, 0, y, width);
        };

        Window_ShopNumber.prototype.drawTotalAfter = function(width, y) {
            let value = this.getTotalCurrency();
            value += (this._price * this._number) * (this.isSelling() ? 1 : -1);
            this.drawCurrencyValue(Yanfly.Util.toGroup(value), this._currencyUnit, 0, y, width);
        };

        Window_ShopNumber.prototype.drawNumber = function() {
            const x = this.cursorX();
            const y = this.itemNameY();
            const width = this.cursorWidth() - this.itemPadding() * 2;
            this.drawText(this._number, x, y, width, "right");
        };

        Window_ShopNumber.prototype.drawMultiplicationSign = function() {
            const sign = '\u00d7';
            const signX = this.cursorX() - this.textWidth(sign) - 4;
            const signY = this.itemNameY();
            this.drawText(sign, signX, signY, this.textWidth(sign) + 8);
        };

        Window_ShopNumber.prototype.itemRect = function(index) {
            const rect = new Rectangle();
            rect.x = this.cursorX();
        ...
        Window_ShopNumber.prototype.updateCursor = function() {
            if (this._cursorAll) {
                ...
                this.setCursorRect(
                    this.itemRect(this.index()).x,
                    this.itemRect(this.index()).y,
                    this.itemRect(this.index()).width,
                    this.itemRect(this.index()).height
                );
            } else {
...
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for YEP_ShopMenuCore
//=============================================================================
...
//=============================================================================
// FOSSIL_PATCH_SHK :: START Patch for YEP_StatAllocation
//=============================================================================
// Date: 2025/08/10
// Reason: Fixes an issue where the help window in the stat allocation scene
// would fail to display. This patch overrides the creation method to ensure the
// window is properly instantiated, positioned, and made visible.
//-----------------------------------------------------------------------------
Scene_StatAllocation.prototype.createHelpWindow = function() {
  // Reason: This override reimplements the help window creation from scratch.
  // The original plugin's method may be incompatible or incomplete in the current
  // engine version, so this ensures the window is reliably created, positioned
  // at the top of the screen, and added to the scene's window layer.
...
//=============================================================================
// FOSSIL_PATCH_SHK :: END Patch for YEP_StatAllocation
//=============================================================================
		...
		PluginManager.setup = function(plugins) {
			...
			for (const plugin of plugins) {
				...
				if (plugin.status && !this._scripts.includes(plugin.name)) {
					...
					//Add Functionality For Dynamic Plugin Names (by SnakekillerX)
					if(plugin.parameters.PluginName){
						plugin.parameters.PluginName = plugin.name;
						window.currentPluginProcessed = plugin.parameters.PluginName;
					}
